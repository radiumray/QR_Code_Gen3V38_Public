...
void setMod2(int m, int x, int y, int c) {
  ...
  if (m == 1) {
    ...
    if (c == 0) {
//      Tft.fillRect((y - 1)*qrS + xOffs, (qrM - x)*qrS + yOffs, qrS, qrS, ST7735_BLACK);
      Tft.fillRect((y - 1)*qrS + xOffs, (qrM - x)*qrS + yOffs, qrS, qrS, ST7735_WHITE);
    } else {
//      Tft.fillRect((y - 1)*qrS + xOffs, (qrM - x)*qrS + yOffs, qrS, qrS, ST7735_WHITE);
      Tft.fillRect((y - 1)*qrS + xOffs, (qrM - x)*qrS + yOffs, qrS, qrS, ST7735_BLACK);
    }
...
void setBuf(int x, int y, int c) {
  ...
  if (x > 28 || x < 0) {
    x = 0; //dont allow overrange
  }
  if (y > 28 || y < 0) {
    y = 0; // dont allow overrange
  }
  if (c == 0) {
